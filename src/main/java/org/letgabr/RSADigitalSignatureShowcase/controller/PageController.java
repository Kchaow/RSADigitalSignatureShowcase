...
public class PageController {
    ...
    public String testing(Model model, HttpServletRequest httpServletRequest) {
        ...
        model.addAttribute("p", userSession.map(x -> x.getRsaCryptoSystem().getP().toString()).orElseGet(() -> ""));
        model.addAttribute("q", userSession.map(x -> x.getRsaCryptoSystem().getQ().toString()).orElseGet(() -> ""));
        ...
        model.addAttribute("e", userSession.map(x -> x.getRsaCryptoSystem().getPublicKey().toString()).orElseGet(() -> ""));
        model.addAttribute("d", userSession.map(x -> x.getRsaCryptoSystem().getPrivateKey().toString()).orElseGet(() -> ""));
        model.addAttribute("n", userSession.map(x -> x.getRsaCryptoSystem().getN().toString()).orElseGet(() -> ""));
//        model.addAttribute("connectedId", userSession.filter((x) -> x.getConnectionStatus().getStatus().equals("connected"))
//                .map(x -> x.getConnectionStatus().getUserId()).orElseGet(() -> ""));
        return "index";
    }
}
